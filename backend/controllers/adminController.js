...
// @desc    Get all counseling leads
// @route   GET /api/admin/counseling
// @access  Private (Admin)
exports.getCounselingLeads = asyncHandler(async (req, res) => {
  const CounselingLead = require('../models/CounselingLead');
  const leads = await CounselingLead.find().sort({ timestamp: -1 });
  console.log('>>> ADMIN FETCHED COUNSELING LEADS:', leads.length);
  res.status(200).json({
    success: true,
    leads
  });
